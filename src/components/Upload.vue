  ...
  methods: {
    uploadSong(event) {
      ...
      files.forEach((file) => {
        ...
        const songsRef = storageRef.child(`songs/${file.name}`); // vue-music-9e624.appspot.com/songs/example.mp3
        const task = songsRef.put(file); // initialize upload to firebase
        ...
        task.on(
          ...
          async () => {
            ...
            const songSnapShot = await songRef.get(); // getting the snapshot
    ...
    // ref way to handle this since nav guard dont work in child comp
    // cancelUpload() {
    //   this.uploads.forEach((el) => el.task.cancel());
    // },
  },
  beforeUnmount() {
    this.uploads.forEach((el) => el.task.cancel());
